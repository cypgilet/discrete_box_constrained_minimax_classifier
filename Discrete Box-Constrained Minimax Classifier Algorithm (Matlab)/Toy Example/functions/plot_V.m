function [ R_DBC ] = plot_V(parameters,pHat,T,piTrain,Box)
%--------------------------------------------------------------------------
...
if K == 2
    pi1 = sort(unique([0:0.01:1 piTrain(1)]));
    ...
    hold on
    plot(pi1Box,div_DBC_U,'-','LineWidth',1.5,'Color',[0.3 0.75 0.96]);
    hold on
    plot([piTrain(1) piTrain(1)],[0 V(piTrain(1)==pi1)],'LineWidth',0.5,'color','k')
    set(gca,'TickLabelInterpreter', 'latex');
    set(gca,'XTick',[0 piTrain(1) 1], 'xticklabel',{0 '$\hat{\pi}_1$' 1})
    grid on
